...
class DroneControlNode(Node):
    def __init__(self):
        ...
        self.vel_pub = self.create_publisher(
            Twist, '/uav3/setpoint_velocity/cmd_vel_unstamped', 10)
        self.local_pos_pub = self.create_publisher(
            PoseStamped, '/uav3/setpoint_position/local', 10)
        self.smooth_path_pub = self.create_publisher(
            Path, '/smooth_path', 10)

        # ROS Subscribers
        self.vel_sub = self.create_subscription(
            TwistStamped, '/uav3/local_position/velocity_local', 
        ...
        self.status_sub = self.create_subscription(
            State, '/uav3/state', self.status_callback, 10)
        ...
        # ROS Clients
        self.setmode_client = self.create_client(SetMode, '/uav3/set_mode')
...
def main():
    ...
    left_matrix_rotate(node, drone_id=3, angle_deg=120.0, tolerance=0.3, timeout=20.0, inter_drone_distance=3.0)
